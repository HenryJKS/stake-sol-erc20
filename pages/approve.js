  ...
  onSubmit = async (event) => {
    ...
    try {
      await mytoken.methods
        .approve(this.state.address, (this.state.amount * 10 ** decimals).toString())
        .send({
          from: accounts[0],
        });
      this.setState({
        successApprove: "Success to Approve, check the list of approvers",
        address: "",
        amount: "",
        loadingButton: false,
      });
    ...
    return (
      <Layout>
        <Container className={styles.containerApprove}>
          ...
          <List as="ul">
            ...
            <ListItem as="li">
              Address to Approve:{" "}
              <a
                href={`https://sepolia.etherscan.io/address/${process.env.NEXT_PUBLIC_ADDRESS_STAKE}`}
              >
        ...
        >
          ...
          <Button primary loading={this.state.loadingButton}>
            Approve
          </Button>
          <Message error header="Error" content={this.state.messageError} />
          <Modal
